...
import { fetchRecommendedCars } from "../../store/recommendedCarsSlice";

function RecommendationCarsSection() {
  const dispatch = useDispatch();
  const cars = useSelector((state) => state.recommendedCars.recommendedCars);
  const status = useSelector((state) => state.recommendedCars.status);

  useEffect(() => {
    if (status === "idle") {
      dispatch(fetchRecommendedCars());
    }
  }, [status, dispatch]);
  ...
  return (
    <section className="bg-gray-100 py-8 px-4 sm:px-8 lg:px-16">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-bold text-gray-800">Recommendation Cars</h2>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {cars.map((car, index) => (
          <div key={index} className="bg-white p-4 rounded-lg shadow-md">
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                {car.attributes.OldPrice && (
                  <span className="text-sm text-gray-500 line-through ml-2">
                    ${car.attributes.OldPrice}.00
                  </span>
                )}
              </div>
